...
export class GenericAuthProvider implements AuthProvider {
    ...
    readonly callback: express.RequestHandler = (request, response, next) => {
        ...
        const passportAuthHandler = passport.authenticate(this.strategy as any, async (...[err, user, info]: Parameters<VerifyCallback>) => {
            /*
            ...
            if (info) {
                ...
                if (VerifyResultWithIdentity.is(info) || (VerifyResultWithUser.is(info) && info.termsAcceptanceRequired)) {
                    ...
                    log.info(context, `(${strategyName}) Redirect to /api/tos`, { info });
                    await redirectToTos();
                    return;
                } else  {
                    const { user, elevateScopes } = info as VerifyResultWithUser;
                    log.info(context, `(${strategyName}) Directly log in and proceed.`, { info });
    ...
    protected async verify(req: express.Request, accessToken: string, refreshToken: string | undefined, tokenResponse: any, _profile: undefined, done: VerifyCallback) {
        ...
        try {
            ...
            if (currentGitpodUser) {
                ...
                await this.userService.updateUserEnvVarsOnLogin(currentGitpodUser, envVars);

                hints = <VerifyResultWithUser>{
                ...
                hints = <VerifyResultWithIdentity>{
                    candidate,
                    token,
                    authUser,
                    envVars,
